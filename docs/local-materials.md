...
The receipt also stores a separate private selection secret. After the same exact registration confirmation, the Host sends it in `X-Rulith-Material-Selection-Key` to the Agent task endpoint. The Agent forwards it only on an `ApplyAction` whose `args` contain one direct parameter value that is an exact `{ref, digest}` object matching an attachment on that task. The secret is omitted from JSON, model messages, events, and conversation storage; an exact task retry binds to its digest. This key lives only for the current Agent task turn. If document clarification postpones the Action to a later message, this transport does not supply the key then. A later-turn path needs an explicit durable submission and current Case binding at the Host; the conversation key cannot establish that binding. Until that path and Gateway/Core validation are integrated, this transport does not grant material execution.
