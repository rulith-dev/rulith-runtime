...
const RULITH_WORKER_CONTRACT_SOURCE_COMMIT = '26f0d4782cb1ba78bc63f4283ec4bbbfd95ac014'
...
async function handDbExec(t, sources = SOURCE_CONTEXT) {
  ...
  if (error !== undefined) return error
  let r
  try { r = await databaseDriver.run(dsn, sql, t.values ?? []) } catch (error) {
    if (t.inputRolesV2) throw new ResultDeliveryError(`Action v2 database outcome is unknown after submission: ${String(error?.message ?? error).slice(0, 160)}`)
    throw error
  }
  if (t.inputRolesV2) {
    if (r.rowCount === 0 && r.command === 'UPDATE') throw new Error('Action v2 update matched zero rows; target or base version changed')
    if (r.rowCount !== 1 || r.command !== 'UPDATE') {
      throw new ResultDeliveryError('Action v2 update effect is unknown or affected multiple rows; reconcile the external Source before any new invocation')
    }
    if (!Array.isArray(r.rows) || r.rows.length !== 1) {
      throw new ResultDeliveryError('Action v2 update returned no unique result row; reconcile the external Source')
    }
    try { resultFactsFromRows(t, r.rows) } catch (error) {
      throw new ResultDeliveryError(`Action v2 update result cannot support its declared returns: ${String(error?.message ?? error).slice(0, 160)}`)
    }
  }
