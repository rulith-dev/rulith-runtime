...
const RULITH_WORKER_CONTRACT_SOURCE_COMMIT = 'ed351cd2ed3d5403003a89fb71ed56bf6cca1aba'
...
const SHADOW_ACTION_FIELDS = Object.freeze(['invocationId', 'actionId', 'grant'])
...
function handRun(t, args, context = {}, sources = SOURCE_CONTEXT) {
  return new Promise((finish, reject) => {
    ...
    const env = {
      ...
      ...(context.invocationId ? { RULITH_INVOCATION_ID: String(context.invocationId) } : {}),
 ...
 *
 * This Runtime does not upload yet: the producer endpoint and the Gateway's permission
 * lookup are still being wired. What it does do is refuse correctly, by name, and never
 * treat an over-limit or unknown result as a success or as something to retry.
 */
...
export function workerArtifactReference(value) {
  ...
  return typeof value.ref === 'string' && value.ref !== '' ? { ref: value.ref } : undefined
}
...
async function handleEvidence(w) {
  ...
  say(`● Claimed material request ${w.material} for ${w.tool} × ${w.norm}; fetching…`, 'claimed',
    { kind: 'material', id: w.material, tool: w.tool })
  let facts = []
  let exhibit
  let err
  const stopRenewing = keepLeaseAlive()
  ...
  const rep = await work({ kind: 'ReportWork', workType: 'evidence', material: w.material, facts, ...(exhibit ? { exhibit } : {}) }, fetchedUnder)
...
async function handleAction(w) {
  ...
  let reason
  try {
    const executed = await execute(action, invocationArgs(resolved, w), { [action]: resolved }, SOURCE_CONTEXT, { invocationId: invocation })
