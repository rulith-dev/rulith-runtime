...
export async function pgRun(dsn, sql, values = []) {
  ...
  try {
    ...
    return { rows: r.rows ?? [], rowCount: r.rowCount, command: String(r.command ?? '') }
  } finally { await client.end().catch(() => {}) }
}

/** PostgreSQL enforces the read fence even when a SELECT calls a volatile function. */
export async function pgRunReadOnlyWithClient(client, sql, values) {
  try {
    await client.connect()
    await client.query('BEGIN READ ONLY')
    try {
      const raw = await client.query({ text: sql, values })
      const result = { rows: raw.rows ?? [], rowCount: raw.rowCount, command: String(raw.command ?? '') }
...
export async function pgRunReadOnly(dsn, sql, values = []) {
  let Client
  try { ({ Client } = await import('pg')) } catch {
    throw new Error('The pg driver is not installed. Database tools require: npm i pg')
  }
...
/** Keep the v2 UPDATE and its result checks inside one PostgreSQL transaction. */
export async function pgRunV2UpdateWithClient(client, sql, values, validateResult) {
  try {
    await client.connect()
...
export async function pgRunV2Update(dsn, sql, values, validateResult) {
  let Client
  try { ({ Client } = await import('pg')) } catch {
    throw new Error('The pg driver is not installed. Database tools require: npm i pg')
  }
  const client = new Client({ connectionString: dsn, connectionTimeoutMillis: 5000, query_timeout: 20_000, statement_timeout: 20_000 })
  return pgRunV2UpdateWithClient(client, sql, values, validateResult)
}
 ...
 */
export const databaseDriver = { run: pgRun, runReadOnly: pgRunReadOnly, runV2Update: pgRunV2Update }
...
async function handDbQuery(t, sources = SOURCE_CONTEXT) {
  ...
  const r = await databaseDriver.runReadOnly(dsn, sql, t.values ?? [])
...
async function handDbExec(t, sources = SOURCE_CONTEXT) {
  ...
  const { dsn, error } = databaseDsn(t, sources)
  if (error !== undefined) return error
  const r = t.inputRolesV2
    ? await databaseDriver.runV2Update(dsn, sql, t.values ?? [], result => {
      if (result.rowCount === 0 && result.command === 'UPDATE') throw new Error('Action v2 update matched zero rows; target or base version changed')
      if (result.rowCount !== 1 || result.command !== 'UPDATE') throw new Error('Action v2 update did not affect exactly one UPDATE row')
      if (!Array.isArray(result.rows) || result.rows.length !== 1) throw new Error('Action v2 update did not return exactly one result row')
      resultFactsFromRows(t, result.rows)
    })
    : await databaseDriver.run(dsn, sql, t.values ?? [])
  const result = `sql:${cls} ${verb} ok rows=${r.rowCount} ${r.command}`.slice(0, 2000)
  return t.inputRolesV2 ? { result, rows: r.rows } : result
}
...
async function handleAction(w) {
  ...
  try {
    ...
    if (resolved.inputRolesV2 && w.toolContractId !== resolved.toolContractId) {
      throw new Error('Action v2 Tool contract id differs from the pinned local exec')
    }
...
export function workerToolsOf(raw) {
  ...
  for (const [id, value] of Object.entries(raw.tools)) {
    ...
    if (value.sourceTypes.includes('db') && !['db-query', 'db-exec-fenced'].includes(adapter)) {
      throw new Error(`Worker Tool ${id} cannot receive a database Source through ${adapter}; database Sources require a fixed-SQL adapter`)
    }
    ...
    // A declared kind cannot make a SQL mutation look like a read at Source lock or Poll.
    if ((adapter === 'db-exec-fenced' && value.kind !== undefined && value.kind !== 'write')
        || (adapter === 'db-query' && value.kind !== undefined && value.kind !== 'read')) {
      throw new Error(`Worker Tool ${id}.kind conflicts with the ${adapter} database adapter`)
    }
