...
function renderCases(){const root=$('cases'),oldTop=root.scrollTop;let html='<div class="case '+(state.active===''&&!state.fresh?'active':'')+'" data-case="">All activity</div>';for(const c of [...state.cases.values()].reverse())html+='<div class="case '+(state.active===c.id?'active':'')+'" data-case="'+esc(c.id)+'"><span>'+esc(c.title)+'</span><small>'+esc(c.status)+' · '+esc(c.id)+'</small></div>';if(html===state.lastCases)return;root.innerHTML=html;state.lastCases=html;root.scrollTop=oldTop;root.querySelectorAll('.case').forEach((n)=>n.onclick=()=>{sayCompose('');state.fresh=false;state.active=n.dataset.case;if(n.dataset.case)state.session=n.dataset.case;showNotes();renderAttachments();render(true);if(EMBEDDED)closeModal('convmodal')})}
...
const es=new EventSource('/events?k='+encodeURIComponent(K));es.onmessage=(m)=>{const e=JSON.parse(m.data);const at=e.historyKey?state.events.findIndex(x=>x.historyKey===e.historyKey):-1;if(at>=0)state.events[at]=e;else if(!e.sequence||!state.events.some(x=>x.sequence===e.sequence&&x.t===e.t))state.events.push(e);state.events.sort((a,b)=>(a.t||a.at||0)-(b.t||b.at||0));remember(e);render()}
...
$('composer').addEventListener('submit',async(ev)=>{
  ...
  const submission={text,...($('casetype').value.trim()?{caseType:$('casetype').value.trim()}:{}),sessionKey:state.session||('ctx-'+crypto.randomUUID()),...(businessKey===undefined?{}:{businessKey}),...(ready.length?{attachments:ready.map((row)=>row.id)}:{})}
...
$('newcase').onclick=()=>{sayCompose('');state.session='';state.active='';state.fresh=true;state.view='case';newDrafts+=1;newDraftKey='new:'+newDrafts;showNotes();renderAttachments();render(true);if(EMBEDDED)closeModal('convmodal');$('prompt').focus()};$('clear').onclick=()=>{state.events=[];render(true)};$('prompt').addEventListener('keydown',(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();$('composer').requestSubmit()}});$('prompt').addEventListener('input',()=>{sayCompose('');saySent('')});
