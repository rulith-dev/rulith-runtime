...
.materialtarget{display:flex;align-items:center;gap:8px;margin:0 2px 8px;color:var(--dim);font-size:12px}.materialtarget[hidden]{display:none}.materialtarget select{min-width:0;max-width:320px;padding:5px 8px;background:var(--panel);color:var(--text);border:1px solid var(--line);border-radius:7px}
...
function draftOf(owner){const key=owner===undefined?draftOwner():owner,held=drafts.get(key)
  if(held)return held
  const made={rows:[],error:'',sent:'',text:'',caseType:'',businessKey:'',targetCaseId:undefined};drafts.set(key,made);return made}
...
// This list comes only from the selected conversation's observed Case events. A
// process-wide Agent focus or another conversation is never a material target.
function materialTargets(){
  if(state.fresh||!state.session)return []
  const roots=new Map()
  for(const event of state.events){
    if(caseOf(event)!==state.session)continue
    if(event.type==='focus'){
      roots.clear()
      for(const row of Array.isArray(event.roots)?event.roots:[]){
        const id=String(row.caseId||'')
        if(/^[A-Za-z0-9:_-]{1,256}$/.test(id)&&row.status==='running')roots.set(id,row.contact==='observed')
      }
    }else if(event.type==='case-open'&&event.ok!==false){
      const id=String(event.caseId||'')
      if(/^[A-Za-z0-9:_-]{1,256}$/.test(id))roots.set(id,true)
    }else if(event.type==='case-state'){
      const id=String(event.caseId||'')
      if(event.caseStatus==='closed'||event.caseStatus==='paused'||event.caseStatus==='unavailable')roots.delete(id)
      else if(event.caseStatus==='running'&&/^[A-Za-z0-9:_-]{1,256}$/.test(id))
        roots.set(id,event.contact!=='not-refreshed')
    }else if(event.type==='case-closed'||event.type==='case-unfocused')roots.delete(String(event.caseId||''))
  }
  return [...roots].map(([id,observed])=>({id,observed}))
}
function renderMaterialTarget(){
  const wrap=$('materialtargetwrap'),select=$('materialtarget'),draft=draftOf(),targets=materialTargets()
  wrap.hidden=attachRows().length===0
  if(wrap.hidden)return
  let chosen=draft.targetCaseId
  if(chosen===undefined)chosen=targets.length===1&&targets[0].observed?targets[0].id:targets.length?'__choose__':''
  const options=[]
  if(targets.length>1||targets.some(target=>!target.observed))options.push(['__choose__','Choose a Case for these files'])
  for(const target of targets)options.push([target.id,'Existing Case · '+target.id+(target.observed?'':' · status not refreshed')])
  options.push(['','Start a new Case'])
  if(chosen&&!targets.some(target=>target.id===chosen)&&chosen!=='__choose__')options.unshift(['__missing__','Previously selected Case is no longer in focus'])
  ...
  select.value=chosen&&!targets.some(target=>target.id===chosen)&&chosen!=='__choose__'?'__missing__':chosen
}
$('materialtarget').addEventListener('change',()=>{draftOf().targetCaseId=$('materialtarget').value;sayCompose('')})
...
function adoptDraft(from,to){const moved=draftOf(from),held=drafts.get(to)
  if(held&&held!==moved){
    ...
    if(held.text?.trim()||held.rows.length){Object.assign(moved,{text:held.text,caseType:held.caseType,businessKey:held.businessKey,targetCaseId:held.targetCaseId,error:held.error});moved.rows.push(...held.rows)}
  }
...
$('composer').addEventListener('submit',async(ev)=>{
  ...
  const targetCaseId=ready.length?$('materialtarget').value:''
  if(targetCaseId==='__choose__'||targetCaseId==='__missing__'){
    sayCompose(targetCaseId==='__choose__'?'Choose which Case receives these files, or start a new Case.':'The selected Case is no longer in this conversation’s focus. Choose another Case or start a new one.')
    $('materialtarget').focus();return
  }
  ...
  const submission={text,...(history.confirmedFor?{historyModelDestination:history.confirmedFor}:{}),...($('casetype').value.trim()?{caseType:$('casetype').value.trim()}:{}),sessionKey:state.session||('ctx-'+crypto.randomUUID()),...(businessKey===undefined?{}:{businessKey}),...(ready.length?{attachments:ready.map((row)=>row.id)}:{}),...(ready.length&&targetCaseId?{caseId:targetCaseId}:{})}
