...
$('composer').addEventListener('submit',async(ev)=>{
  ...
  const r=await fetch('/cases?k='+encodeURIComponent(K),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text,...($('casetype').value.trim()?{caseType:$('casetype').value.trim()}:{}),...(state.session?{sessionKey:state.session}:{}),...(businessKey===undefined?{}:{businessKey}),...(ready.length?{attachments:ready.map((row)=>row.id)}:{})})}).then((x)=>x.json()).catch(()=>null)
