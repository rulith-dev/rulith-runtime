...
function mergeEvent(e){const at=e.historyKey?state.events.findIndex(x=>x.historyKey===e.historyKey):-1;if(at>=0){if(!state.events[at].historical&&e.historical)return;state.events[at]=e}else if(!e.sequence||!state.events.some(x=>x.sequence===e.sequence&&x.t===e.t))state.events.push(e);remember(e)}
function historyControls(){
  const retry=pendingSends.get(draftOwner())?.interrupted===true
  $('newattempt').hidden=!retry
  $('historybar').hidden=(state.fresh||!state.active)&&!retry&&!(history.expectedModel&&history.confirmedFor!==history.expectedModel)
...
async function readHistory(key,older=false){
  ...
  if(!key){history.before=null;history.selectedArchived=false;historyControls();return}
...
async function selectConversation(key){
  sayCompose('');state.fresh=false;state.active=key;if(key)state.session=key
  history.before=null;history.expectedModel='';history.confirmedFor='';history.selectedArchived=false
  showNotes();renderAttachments();render(true);if(EMBEDDED)closeModal('convmodal');await readHistory(key)
}
...
$('newcase').onclick=()=>{history.request++;history.selectedArchived=false;history.before=null;history.expectedModel='';history.confirmedFor='';$('newattempt').hidden=true;sayCompose('');state.session='';state.active='';state.fresh=true;state.view='case';newDrafts+=1;newDraftKey='new:'+newDrafts;showNotes();renderAttachments();render(true);if(EMBEDDED)closeModal('convmodal');$('prompt').focus()};$('clear').onclick=()=>{state.events=[];render(true)};$('prompt').addEventListener('keydown',(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();$('composer').requestSubmit()}});$('prompt').addEventListener('input',()=>{sayCompose('');saySent('')});
