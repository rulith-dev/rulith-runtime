...
const channel = process.env.RULITH_CHANNEL ?? process.argv[2]
if (!channel || channel === '__RULITH_CHANNEL__') {
  console.error('usage: node prepare-runtime.mjs <registered-channel-id>')
  process.exit(2)
}
if (!/^[A-Za-z0-9._:@-]{1,200}$/.test(channel)) {
  console.error('channel id contains unsupported characters')
  process.exit(2)
}
const runtime = resolve(process.env.RULITH_CALC_RUNTIME ?? join(HERE, 'runtime'))
mkdirSync(runtime, { recursive: true })

const recipe = readFileSync(join(HERE, 'recipe.template.json'), 'utf8')
  .replaceAll('__RULITH_CHANNEL__', channel)
...
JSON.parse(recipe)
JSON.parse(tools)
writeFileSync(join(runtime, 'recipe.json'), recipe)
...
console.log(JSON.stringify({
  runtime,
  recipe: join(runtime, 'recipe.json'),
