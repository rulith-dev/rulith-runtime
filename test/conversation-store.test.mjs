...
test('restart preserves visible history and turns unfinished work into an interruption without a replay queue', async t => {
...
test('account, Agent and origin isolate local history; a damaged file is preserved and blocks writes', async t => {
  ...
  for (const other of [{ ...owner, accountId: 'b' }, { ...owner, agentId: 'b' }, { ...owner, origin: 'https://other.test' }]) {
    const isolated = await openConversations(dir, other)
    assert.equal(isolated.snapshot().turns.length, 0); isolated.close()
    assert.throws(() => readConversations(first.file, other), /owner/)
  }
  writeFileSync(first.file, '{truncated')
  first.close()
  await assert.rejects(openConversations(dir, owner), /preserved/)
...
test('real Agent deduplicates accepted messages and restores only conversational text after process restart', async t => {
  ...
  const first = await runAgent({ argv: ['--serve'], env, serveTasks: [body, body, { ...body, text: 'changed' }], waitForServeCompletion: true, model: () => ({ text: 'Blue remembered.', usage: { prompt_tokens: 120, completion_tokens: 9 } }), timeoutMs: 100 })
  ...
  const measured = readConversations(conversationFile(dir, owner), owner).turns[0].usage
  assert.equal(measured.calls, 1); assert.equal(measured.inputTokens, 120); assert.equal(measured.outputTokens, 9); assert.equal(measured.unknownUsageCalls, 0)
  const next = { text: 'What color?', sessionKey: 'chat-1', requestId: 'request-history-0002' }
  const second = await runAgent({ argv: ['--serve'], env: { ...env, RULITH_SERVE_PORT: String(await freePort()) }, serveTasks: [next, responses => ({ ...next, historyModelDestination: responses[0].body.modelService })], waitForServeCompletion: true, model: () => 'Blue.', timeoutMs: 100 })
  assert.deepEqual(second.serveStatuses, [409, 202])
...
test('a history write failure refuses admission before any model or Board tool executes', async t => {
  ...
  const run = await runAgent({ argv: ['--serve'], env: { RULITH_CONVERSATION_DIR: blocked, RULITH_CONVERSATION_OWNER: JSON.stringify(owner), RULITH_SERVE_KEY: 'store-failure', RULITH_SERVE_PORT: String(await freePort()) }, model: () => 'wrong', timeoutMs: 1000 })
  assert.equal(run.code, 5)
  assert.match(run.stderr, /history is locked or unavailable/)
  assert.equal(run.modelRequests.length, 0)
...
test('the workbench reads stopped Agent history through its authenticated event stream', async t => {
  const dir = fixture(t), store = await openConversations(join(dir, 'conversations'), owner); t.after(() => store.close())
